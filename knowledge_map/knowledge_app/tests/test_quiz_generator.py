        ...
        self.sample_topics = [
            {
                ...
                "sentences": [
                    f"This is sentence {k} about topic {i}." for k in range(3)
                ],
            }
            for i in range(4)
        ]
...
class GenerateFillInBlankTestCase(QuizGeneratorTestCase):
    ...
    def test_uses_second_sentence_if_available(self):
        ...
        self.assertNotEqual(
            question["question"], "The main concept in this topic is: _____"
        )
...
class GenerateQuizTestCase(QuizGeneratorTestCase):
    ...
    def test_cycles_through_question_types(self):
        ...
        quiz = generate_quiz(
            self.sample_topics, num_questions=9, question_types=question_types
        )
    ...
    def test_uses_specified_question_types(self):
        ...
        quiz = generate_quiz(
            self.sample_topics, num_questions=6, question_types=question_types
        )
    ...
    def test_matching_not_included_by_default(self):
        ...
        quiz = generate_quiz(
            self.sample_topics, num_questions=5, include_matching=False
        )
    ...
    def test_matching_requires_four_topics(self):
        ...
        quiz = generate_quiz(
            self.sample_topics[:2], num_questions=5, include_matching=True
        )
...
class GenerateQuizFromTextTestCase(QuizGeneratorTestCase):
    ...
    def test_respects_difficulty_level(self, mock_openai_class):
        ...
        with patch("knowledge_app.services.quiz_generator.Question"):
            generate_quiz_from_text(MagicMock(), self.sample_text, difficulty="hard")
    ...
    def test_sets_question_order(self, mock_openai_class):
        ...
        with patch(
            "knowledge_app.services.quiz_generator.Question", mock_question_model
        ):
