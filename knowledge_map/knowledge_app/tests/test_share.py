...
class ShareMapTest(TestCase):
    ...
    def test_generate_public_link_creates_shared_map(self):
        ...
        self.client.post(
            reverse("share_map", args=[self.knowledge_map.id]), {"share_type": "public"}
        )
        self.assertEqual(
            SharedMap.objects.filter(
    ...
    def test_public_link_is_accessible_without_login(self):
        ...
        response = self.client.get(
            reverse("view_shared_map", args=[shared.share_token])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_share_with_user_creates_shared_map(self):
        ...
        self.client.post(
            reverse("share_map", args=[self.knowledge_map.id]),
            {"share_type": "user", "username": "otheruser"},
        )
        self.assertTrue(
            SharedMap.objects.filter(
                knowledge_map=self.knowledge_map, shared_with=self.other_user
            ).exists()
        )
    ...
    def test_share_with_nonexistent_user_returns_404(self):
        ...
        response = self.client.post(
            reverse("share_map", args=[self.knowledge_map.id]),
            {"share_type": "user", "username": "doesnotexist"},
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_share_with_yourself_returns_400(self):
        ...
        response = self.client.post(
            reverse("share_map", args=[self.knowledge_map.id]),
            {"share_type": "user", "username": "owner"},
        )
