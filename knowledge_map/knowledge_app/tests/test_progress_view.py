...
class ProgressViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.quiz = Quiz.objects.create(
            user=self.user, title="Test Quiz", difficulty="medium"
        )
    ...
    def test_score_80_or_above_is_mastered(self):
        QuizAttempt.objects.create(
            quiz=self.quiz, user=self.user, score=85, correct_count=1, total_questions=1
        )
        response = self.client.get(reverse("progress"))
        quiz_data = response.context["quiz_data"]
    ...
    def test_score_60_to_79_is_learning(self):
        QuizAttempt.objects.create(
            quiz=self.quiz, user=self.user, score=70, correct_count=1, total_questions=1
        )
        response = self.client.get(reverse("progress"))
        quiz_data = response.context["quiz_data"]
    ...
    def test_score_below_60_is_needs_practice(self):
        QuizAttempt.objects.create(
            quiz=self.quiz, user=self.user, score=45, correct_count=1, total_questions=1
        )
        response = self.client.get(reverse("progress"))
        quiz_data = response.context["quiz_data"]
    ...
    def test_highest_score_tracked(self):
        QuizAttempt.objects.create(
            quiz=self.quiz, user=self.user, score=50, correct_count=1, total_questions=1
        )
        QuizAttempt.objects.create(
            quiz=self.quiz, user=self.user, score=90, correct_count=1, total_questions=1
        )
        response = self.client.get(reverse("progress"))
        quiz_data = response.context["quiz_data"]
    ...
    def test_status_based_on_latest_not_highest(self):
        ...
        QuizAttempt.objects.create(
            quiz=self.quiz, user=self.user, score=40, correct_count=1, total_questions=1
        )
        response = self.client.get(reverse("progress"))
        quiz_data = response.context["quiz_data"]
    ...
    def test_trend_diff_positive_on_improvement(self):
        QuizAttempt.objects.create(
            quiz=self.quiz, user=self.user, score=60, correct_count=1, total_questions=1
        )
        QuizAttempt.objects.create(
            quiz=self.quiz, user=self.user, score=80, correct_count=1, total_questions=1
        )
        response = self.client.get(reverse("progress"))
        quiz_data = response.context["quiz_data"]
