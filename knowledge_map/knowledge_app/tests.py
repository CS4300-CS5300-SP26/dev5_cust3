...
class KnowledgeMapViewsTest(TestCase):
    ...
    # VIEW MAP TESTS
    # ----------------------
    def test_view_map_success(self):
        km = KnowledgeMap.objects.create(
            user=self.user,
        ...
        # Create topics
        t1 = TopicNode.objects.create(label="Topic 1", summary="Summary 1", knowledge_map=km)
        t2 = TopicNode.objects.create(label="Topic 2", summary="Summary 2", knowledge_map=km)
        ...
        response = self.client.get(reverse('view_map', args=[km.id]))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'knowledge_app/view_map.html')

        # Check nodes and edges in context
    ...
    def test_view_map_permission_denied(self):
        km = KnowledgeMap.objects.create(
        ...
        response = self.client.get(reverse('view_map', args=[km.id]))
        self.assertEqual(response.status_code, 404)  # should not access others' maps
    ...
    def test_map_status_success(self):
        ...
        response = self.client.get(reverse('map_status', args=[km.id]))
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(response.content, {'status': 'processing'})

    def test_map_status_permission_denied(self):
        km = KnowledgeMap.objects.create(
            user=self.other_user,
            uploaded_file=self.uploaded_file,
            title="Hidden Map",
