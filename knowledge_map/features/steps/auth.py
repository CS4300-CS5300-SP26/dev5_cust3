...
def step_register(context, username, password):
    context.response = context.client.post(
        reverse("register"), {"username": username, "password": password}, follow=True
    )
...
def step_login(context, username, password):
    context.response = context.client.post(
        reverse("login"), {"username": username, "password": password}, follow=True
    )
...
def step_redirected_to_home(context):
    ...
    assert (
        "KnowledgeMap" in context.response.content.decode()
...
def step_redirected_to_login(context):
    assert context.response.status_code == 200
    assert (
        "Log In" in context.response.content.decode()
...
def step_see_text(context, text):
    assert (
        text in context.response.content.decode()
