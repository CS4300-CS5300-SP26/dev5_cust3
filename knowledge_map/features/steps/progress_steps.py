...
def step_quiz_no_attempts(context):
    context.quiz = Quiz.objects.create(
        user=context.user, title="Test Quiz", difficulty="medium"
    )
...
def step_quiz_score_80(context):
    quiz = Quiz.objects.create(
        user=context.user, title="Mastered Quiz", difficulty="easy"
    )
    QuizAttempt.objects.create(
        quiz=quiz, user=context.user, score=80, correct_count=4, total_questions=5
    )
