...
# from knowledge_app.services.quiz_generator import generate_matching, generate_quiz_from_text
...
def step_verify_min_choices(context, min_count):
    ...
    for question in context.quiz:
        assert (
            len(question["choices"]) >= min_count
...
def step_verify_tf_choices(context, count):
    for question in context.quiz:
        ...
        assert set(choices) == {
            True,
            False,
        }, f"Choices for {question['id']} should be [True, False], got {choices}"
...
def step_generate_complete_quiz(context):
    ...
    context.quiz = generate_quiz(
        context.topics, num_questions=len(context.topics) * 2, include_matching=True
    )
...
def step_verify_matching_pairs(context, count):
    ...
    for question in context.quiz:
        if question["type"] == "matching":
            assert (
                "pairs" in question
            ), f"Matching question {question['id']} has no pairs"
            assert (
                len(question["pairs"]) >= count
...
def step_verify_empty_quiz(context):
    ...
    assert (
        len(context.quiz) == 0
    ), f"Expected empty quiz, got {len(context.quiz)} questions"
...
def step_verify_unique_ids(context):
    ...
    assert len(ids) == len(set(ids)), f"Question IDs are not unique: {ids}"
